job "example" {
  ...
  group "g1" {
    ...
    # render it properly in the main task, because consul-template does not do
    # a render pass over template dependency data.
    # For someone using terraform to generate the files, they could write the
    # template data to anyplace that can be served by go-getter—git, mercurial,
    # s3, simple URLs, etc—and use the `source` attribute of the template block
    # as shown below.
    ...
    task "init_tpl" {
      ...
      template {
        data = <<EOH
{{ with nomadVar "nomad/jobs/example/g1/template_sidecar" }}
# Retrieved from nomad/jobs/example/g1/template_sidecar
{{.template}}{{end}}
      EOH
